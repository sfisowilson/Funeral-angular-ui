...
export class OnboardingStepConfigurationClient {
    ...
    getEnabledSteps(): Observable<EnabledOnboardingStepLite[]> {
        if (!this.enabledSteps$) {
            ...
            this.enabledSteps$ = this.http.post<EnabledOnboardingStepLite[]>(url, {}).pipe(shareReplay({ bufferSize: 1, refCount: true }));
        }
    ...
    getAllSteps(): Observable<EnabledOnboardingStepLite[]> {
        if (!this.allSteps$) {
            ...
            this.allSteps$ = this.http.post<EnabledOnboardingStepLite[]>(url, {}).pipe(shareReplay({ bufferSize: 1, refCount: true }));
        }
