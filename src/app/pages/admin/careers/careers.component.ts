...
export class CareersComponent implements OnInit {
    ...
    applyFilters(): void {
        ...
        if (this.searchTerm) {
            ...
            filtered = filtered.filter((c) => c.jobTitle.toLowerCase().includes(search) || c.department.toLowerCase().includes(search) || c.location.toLowerCase().includes(search) || c.description.toLowerCase().includes(search));
        }
    ...
    editCareer(career: Career): void {
        ...
        const deadlineDate = career.applicationDeadline instanceof Date ? career.applicationDeadline.toISOString().split('T')[0] : new Date(career.applicationDeadline).toISOString().split('T')[0];
    ...
    saveCareer(): void {
        ...
        const operation = this.isEditMode ? this.careerService.updateCareer(careerData.id, careerData) : this.careerService.createCareer(careerData);
