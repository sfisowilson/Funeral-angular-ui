...
import { OnboardingContractServiceProxy, GenerateContractRequest, SignContractRequest, ContractGenerationResult } from '../../core/services/service-proxies';
    ...
    template: `
        <div class="container mx-auto p-6 max-w-6xl">
            ...
            <div class="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
                <h3 class="font-semibold text-blue-900 mb-2"><i class="bi bi-info-circle me-2"></i>Electronic Signature Agreement</h3>
                <p class="text-sm text-blue-800">By signing this document electronically, you agree that:</p>
                ...
                <p class="text-xs text-blue-700 mt-3">As per the Electronic Communications and Transactions Act, 2002 (ECTA), this electronic signature is legally binding.</p>
            </div>
            ...
            <div *ngIf="currentStep === 1" class="card">
                ...
                <p class="text-gray-600 mb-4">Click below to generate your onboarding contract with all the information you've provided.</p>
                <button (click)="generateContract()" [disabled]="generating" class="btn btn-primary">
            ...
            <div *ngIf="currentStep === 2" class="card">
                ...
                <p class="text-gray-600 mb-4">Please review the contract carefully before signing. Use the checkbox below to confirm.</p>

                <!-- PDF Viewer -->
                <div class="pdf-container mb-4">
                    <iframe [src]="sanitizedPdfUrl" frameborder="0" class="w-full" style="height: 600px;"> </iframe>
                </div>

                <div class="form-check mb-4">
                    <input type="checkbox" [(ngModel)]="hasReviewedDocument" id="confirmReview" class="form-check-input" />
                    <label for="confirmReview" class="form-check-label"> I have read and understood the terms of this contract </label>
                </div>

                <div class="flex gap-3">
                    <button (click)="currentStep = 1" class="btn btn-secondary">Back</button>
                    <button (click)="currentStep = 3" [disabled]="!hasReviewedDocument" class="btn btn-primary">Proceed to Sign</button>
                </div>
            </div>
            ...
            <div *ngIf="currentStep === 3" class="card">
                ...
                <p class="text-gray-600 mb-4">Draw your signature in the box below using your mouse or touch screen.</p>
                ...
                <div class="flex gap-3 mb-4">
                    <button (click)="clearSignature()" class="btn btn-outline-secondary"><i class="bi bi-arrow-counterclockwise me-2"></i>Clear</button>
                </div>

                <!-- Consent Checkbox -->
                <div class="form-check mb-4">
                    <input type="checkbox" [(ngModel)]="consentGiven" id="confirmConsent" class="form-check-input" />
                    <label for="confirmConsent" class="form-check-label"> I consent to sign this document electronically and understand that it is legally binding </label>
                </div>

                <div class="flex gap-3">
                    <button (click)="currentStep = 2" class="btn btn-secondary">Back</button>
                    <button (click)="signContract()" [disabled]="!consentGiven || !hasSignature() || signing" class="btn btn-success">
            ...
            <div *ngIf="currentStep === 4" class="card text-center">
                ...
                <p class="text-gray-600 mb-4">Your onboarding contract has been signed and is legally binding.</p>
                <p class="text-sm text-gray-500 mb-6">
                    Signed on: {{ signedAt | date: 'full' }}<br />
                    From IP: {{ ipAddress }}
                </p>
                <div class="flex gap-3 justify-center">
                    <button (click)="downloadContract()" class="btn btn-primary"><i class="bi bi-download me-2"></i>Download Contract</button>
                    <button (click)="navigateToProfile()" class="btn btn-outline-primary">Return to Profile</button>
                </div>
            </div>

            <!-- Error Message -->
            <div *ngIf="errorMessage" class="alert alert-danger mt-4"><i class="bi bi-exclamation-triangle me-2"></i>{{ errorMessage }}</div>
        </div>
    `,
    styles: [
        `
