...
export class TenantRegisterWizardComponent extends TenantBaseComponent implements OnInit {
    ...
    onOrganizationTypeSelected(orgType: OrganizationType): void {
        ...
        orgType.questions.forEach((question) => {
            ...
            this.organizationQuestionsForm.addControl(question.id, this.fb.control(defaultValue, validators));
        });
    ...
    calculateDynamicPricing(): void {
        ...
        const yearlyDiscount = plan.yearlyPrice ? Math.round(((monthlyPrice * 12 - yearlyPrice) / (monthlyPrice * 12)) * 100) : 15;
    ...
    async createTenantAndInitiatePayment(): Promise<void> {
        try {
            ...
            const paymentDto: any = {
                ...
                tenantName: formValue.organizationName,
                amount: this.getFinalPrice(),
                currency: 'ZAR',
                description: `${selectedPlan?.name || selectedPlan?.planName} Subscription`,
                returnUrl: `${window.location.origin}/payment-success?email=${encodeURIComponent(formValue.email)}&subdomain=${encodeURIComponent(formValue.domain.toLowerCase())}`,
    ...
    getFinalPrice(): number {
        ...
        let basePrice = this.selectedBillingCycle() === 'yearly' ? plan.yearlyPrice || plan.monthlyPrice * 12 : plan.monthlyPrice;
