...
@Component({
    ...
    standalone: true,
    imports: [CommonModule, FormsModule, TableModule, ButtonModule, DialogModule, InputTextModule, DropdownModule, CalendarModule, CheckboxModule, TooltipModule, ToastModule],
    ...
    template: `
        ...
        <div class="dynamic-repeater">
            ...
            <div class="repeater-header">
                ...
                <button *ngIf="config().allowAdd !== false && !isMaxItemsReached()" pButton type="button" [label]="'Add ' + config().singularLabel" icon="pi pi-plus" (click)="openDialog()" class="p-button-sm p-button-success"></button>
            </div>
            ...
            <!-- Items Table -->
            <p-table *ngIf="items().length > 0" [value]="items()" styleClass="p-datatable-sm mt-3" [tableStyle]="{ 'min-width': '50rem' }">
                ...
                <ng-template pTemplate="body" let-item let-i="rowIndex">
                    <tr>
                        ...
                        <td>
                            <button *ngIf="config().allowEdit !== false" pButton type="button" icon="pi pi-pencil" (click)="editItem(i)" class="p-button-sm p-button-text p-button-rounded" pTooltip="Edit"></button>
                            <button *ngIf="config().allowDelete !== false" pButton type="button" icon="pi pi-trash" (click)="deleteItem(i)" class="p-button-sm p-button-text p-button-rounded p-button-danger" pTooltip="Delete"></button>
                        </td>
                    </tr>
                </ng-template>
                ...
                <ng-template pTemplate="emptymessage">
                    <tr>
                        <td [attr.colspan]="config().fields.length + 1" class="text-center">No {{ config().pluralLabel.toLowerCase() }} added yet.</td>
                    </tr>
                </ng-template>
            </p-table>

            <!-- Dialog for Adding/Editing Items -->
            <p-dialog [(visible)]="showDialog" [header]="dialogTitle()" [modal]="true" [style]="{ width: '600px' }" [draggable]="false" [resizable]="false">
                <div class="form-container">
                    <div *ngFor="let field of config().fields" class="field-group mb-3">
                        ...
                        <!-- Text, Email, Tel -->
                        <input *ngIf="['text', 'email', 'tel'].includes(field.fieldType)" [id]="field.fieldKey" [(ngModel)]="currentItem[field.fieldKey]" [type]="field.fieldType" [placeholder]="field.placeholder || ''" class="form-control" />
                        ...
                        <!-- Select/Dropdown -->
                        <p-dropdown *ngIf="field.fieldType === 'select'" [id]="field.fieldKey" [(ngModel)]="currentItem[field.fieldKey]" [options]="field.options || []" [placeholder]="field.placeholder || 'Select an option'" class="w-100">
                        </p-dropdown>

                        <!-- Checkbox -->
                        <div *ngIf="field.fieldType === 'checkbox'" class="flex align-items-center">
                            <p-checkbox [id]="field.fieldKey" [(ngModel)]="currentItem[field.fieldKey]" [binary]="true"> </p-checkbox>
                ...
                <ng-template pTemplate="footer">
                    <button pButton type="button" label="Cancel" icon="pi pi-times" (click)="closeDialog()" class="p-button-text"></button>
                    <button pButton type="button" label="Save" icon="pi pi-check" (click)="saveItem()" class="p-button-primary"></button>
                </ng-template>
            </p-dialog>
        </div>
    `,
...
export class DynamicRepeaterFieldComponent implements OnInit {
    ...
    dialogTitle(): string {
        return this.editingIndex !== null ? `Edit ${this.config().singularLabel}` : `Add ${this.config().singularLabel}`;
    }
