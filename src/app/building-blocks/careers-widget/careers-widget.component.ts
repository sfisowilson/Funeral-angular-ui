        ...
        <div class="careers-widget" [style.background-color]="config.backgroundColor" [style.--widget-outer-padding.px]="config.padding || 60">
            <div class="container mx-auto" [style.max-width.px]="config.containerWidth || 1200">
                ...
                <div class="text-center mb-5">
                    <h2 class="widget-title" [style.color]="config.titleColor" [style.font-size.px]="config.titleSize || 42" [style.font-weight]="config.titleFontWeight || 700">
                ...
                <div *ngIf="!loading && careers.length > 0 && config.showFilters" class="filters-section mb-4">
                    <div class="row g-3">
                        <div class="col-md-4" *ngIf="config.showDepartmentFilter">
                            <select class="filter-select" [(ngModel)]="selectedDepartment" (change)="applyFilters()" [style.border-color]="config.primaryColor || '#0d6efd'">
                                <option value="">All Departments</option>
                                <option *ngFor="let dept of departments" [value]="dept">{{ dept }}</option>
                            </select>
                        </div>
                        <div class="col-md-4" *ngIf="config.showTypeFilter">
                            <select class="filter-select" [(ngModel)]="selectedType" (change)="applyFilters()" [style.border-color]="config.primaryColor || '#0d6efd'">
                        ...
                        <div class="col-md-4" *ngIf="config.showLocationFilter">
                            <select class="filter-select" [(ngModel)]="selectedLocation" (change)="applyFilters()" [style.border-color]="config.primaryColor || '#0d6efd'">
                ...
                <!-- Careers List -->
                <div *ngIf="!loading && filteredCareers.length > 0" [ngClass]="config.layout === 'grid' ? 'careers-grid' : 'careers-list'">
                    <div *ngFor="let career of filteredCareers" class="career-card" [style.background-color]="config.cardBackgroundColor || '#ffffff'" [style.border-color]="config.cardBorderColor || '#e0e0e0'" (click)="openCareerDetail(career)">
                        <!-- Card Header -->
                        <div class="career-header">
                            <div class="career-title-section">
                                <h3 class="career-title" [style.color]="config.cardTitleColor || '#212529'" [style.font-size.px]="config.cardTitleSize || 22">
                                    {{ career.jobTitle }}
                                </h3>
                            ...
                            <div class="career-badges">
                                <span class="employment-badge" [style.background-color]="getEmploymentTypeColor(career.employmentType)" [style.color]="config.badgeTextColor || '#ffffff'">
                        ...
                        <div class="career-body">
                            <p class="career-description" [style.color]="config.descriptionColor || '#495057'" [style.font-size.px]="config.descriptionSize || 15">
                            ...
                            <div *ngIf="config.showDeadline" class="deadline">
                                ...
                                <span [style.color]="config.deadlineColor || '#6c757d'"> Apply by {{ formatDate(career.applicationDeadline) }} </span>
                            </div>
                        </div>

                        <!-- Card Footer -->
                        <div class="career-footer">
                            <button class="apply-button" [style.background-color]="config.buttonColor || '#0d6efd'" [style.color]="config.buttonTextColor || '#ffffff'" (click)="applyForCareer(career, $event)">
                                {{ config.applyButtonText || 'Apply Now' }}
                                <i class="bi bi-arrow-right ms-2"></i>
                            </button>
                            <button *ngIf="config.showViewDetailsButton" class="details-button" [style.color]="config.detailsButtonColor || '#0d6efd'" (click)="viewDetails(career, $event)">
                ...
                <div *ngIf="!loading && careers.length > 0 && filteredCareers.length === 0" class="text-center py-5">
                    <p [style.color]="config.textColor || '#6c757d'">No positions match your filter criteria.</p>
                </div>
            </div>
        </div>

        <!-- Career Detail Modal (if enabled) -->
        <div *ngIf="config.showDetailModal && selectedCareer" class="career-modal-overlay" (click)="closeModal()">
            <div class="career-modal" (click)="$event.stopPropagation()" [style.background-color]="config.modalBackgroundColor || '#ffffff'">
                <button class="modal-close" (click)="closeModal()" [style.color]="config.closeButtonColor || '#000000'">
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        ...
                        <div class="modal-meta">
                            ...
                            <span class="employment-badge" [style.background-color]="getEmploymentTypeColor(selectedCareer.employmentType)">
                    ...
                    <div class="modal-body">
                        ...
                        <section class="modal-section" *ngIf="selectedCareer.responsibilities && selectedCareer.responsibilities.length > 0">
                            ...
                            <ul>
                                <li *ngFor="let item of selectedCareer.responsibilities" [style.color]="config.descriptionColor || '#6c757d'">
                                    {{ item }}
                                </li>
                            </ul>
                        </section>
                        ...
                        <section class="modal-section" *ngIf="selectedCareer.requirements && selectedCareer.requirements.length > 0">
                            ...
                            <ul>
                                <li *ngFor="let item of selectedCareer.requirements" [style.color]="config.descriptionColor || '#6c757d'">
                                    {{ item }}
                                </li>
                            </ul>
                        </section>
                        ...
                        <section class="modal-section" *ngIf="selectedCareer.benefitsHighlights && selectedCareer.benefitsHighlights.length > 0">
                            ...
                            <ul>
                                <li *ngFor="let item of selectedCareer.benefitsHighlights" [style.color]="config.descriptionColor || '#6c757d'">
                                    {{ item }}
                                </li>
                            </ul>
                        </section>
                        ...
                        <section class="modal-section">
                            <div class="modal-footer-info">
                                <div *ngIf="selectedCareer.salaryRange"><strong>Salary Range:</strong> {{ selectedCareer.salaryRange }}</div>
                                <div><strong>Application Deadline:</strong> {{ formatDate(selectedCareer.applicationDeadline) }}</div>
                            </div>
                        </section>
                    </div>

                    <div class="modal-footer-action">
                        <button class="apply-button-large" [style.background-color]="config.buttonColor || '#0d6efd'" [style.color]="config.buttonTextColor || '#ffffff'" (click)="applyForCareer(selectedCareer, $event)">
                            {{ config.applyButtonText || 'Apply Now' }}
                            <i class="bi bi-arrow-right ms-2"></i>
                        </button>
                    </div>
