...
export class DynamicEntityListWidgetComponent implements OnInit, OnChanges {
    ...
    get filteredRecords(): DynamicEntityRecordDto[] {
        ...
        return this.records.filter((r) => {
            const baseMatch = (r.displayName || '').toLowerCase().includes(term) || (r.externalKey || '').toLowerCase().includes(term);
            if (baseMatch) {
                return true;
            }
    ...
    private loadRecordsIfConfigChanged(force: boolean): void {
        ...
        const changed = force || this.lastEntityTypeKey !== entityTypeKey || this.lastMaxRecords !== maxRecords;
