    ...
    template: `
        <div class="container-fluid p-4">
            <div class="d-flex justify-content-between align-items-center mb-4">
                ...
                <button (click)="showForm = true; resetForm()" class="btn btn-primary"><i class="bi bi-plus-circle me-1"></i> Add Gateway</button>
            </div>
                ...
                <div class="card-body">
                    <form [formGroup]="configForm" (ngSubmit)="saveConfig()">
                        <div class="row g-3">
                            <!-- Provider Selection -->
                            <div class="col-md-6">
                                <label for="provider" class="form-label fw-semibold"> <i class="bi bi-building me-1"></i>Payment Provider * </label>
                                <select id="provider" formControlName="provider" [disabled]="!!editingConfig" class="form-select">
                            ...
                            <div class="col-md-6 d-flex align-items-end">
                                <div class="form-check form-switch">
                                    ...
                                    <label class="form-check-label" for="isTestMode"> <i class="bi bi-bug me-1"></i>Test Mode (Sandbox) </label>
                                </div>
                            </div>

                            <!-- Merchant ID -->
                            <div class="col-md-6">
                                <label for="merchantId" class="form-label fw-semibold"> <i class="bi bi-hash me-1"></i>Merchant ID </label>
                                <input type="text" id="merchantId" formControlName="merchantId" class="form-control" placeholder="Enter merchant ID" />
                            </div>

                            <!-- Site Code (for Ozow) -->
                            <div class="col-md-6">
                                <label for="siteCode" class="form-label fw-semibold"> <i class="bi bi-code-square me-1"></i>Site Code </label>
                                <input type="text" id="siteCode" formControlName="siteCode" class="form-control" placeholder="Enter site code (Ozow only)" />
                                <div class="form-text">Required for Ozow integration</div>
                            </div>

                            <!-- API Key -->
                            <div class="col-md-6">
                                <label for="apiKey" class="form-label fw-semibold"> <i class="bi bi-key me-1"></i>API Key </label>
                                <input type="password" id="apiKey" formControlName="apiKey" class="form-control" placeholder="Enter API key" />
                                <div class="form-text">Leave blank to keep existing</div>
                            </div>

                            <!-- Secret Key -->
                            <div class="col-md-6">
                                <label for="secretKey" class="form-label fw-semibold"> <i class="bi bi-shield-lock me-1"></i>Secret/Merchant Key </label>
                                <input type="password" id="secretKey" formControlName="secretKey" class="form-control" placeholder="Enter secret key" />
                                <div class="form-text">Leave blank to keep existing</div>
                            </div>

                            <!-- Pass Phrase (for PayFast) -->
                            <div class="col-12">
                                <label for="passPhrase" class="form-label fw-semibold"> <i class="bi bi-lock me-1"></i>Pass Phrase (PayFast) </label>
                                <input type="password" id="passPhrase" formControlName="passPhrase" class="form-control" placeholder="Enter pass phrase" />
                                <div class="form-text">Required for PayFast. Leave blank to keep existing.</div>
                            </div>

                            <!-- Webhook URL -->
                            <div class="col-12">
                                <label for="webhookUrl" class="form-label fw-semibold"> <i class="bi bi-arrow-left-right me-1"></i>Webhook URL <span class="badge bg-info">Optional</span> </label>
                                <input type="url" id="webhookUrl" formControlName="webhookUrl" class="form-control" readonly />
                                <div class="form-text">
                                    <i class="bi bi-info-circle me-1"></i>
                                    <strong>This is YOUR backend URL</strong> where the payment gateway sends notifications. Copy this URL and paste it in <strong>PayFast/Ozow's dashboard settings</strong>, not the other way around.
                                </div>
                            </div>

                            <!-- Return URL -->
                            <div class="col-md-6">
                                <label for="returnUrl" class="form-label fw-semibold"> <i class="bi bi-check-circle me-1"></i>Return URL (Success) </label>
                                <input type="url" id="returnUrl" formControlName="returnUrl" class="form-control" />
                                <div class="form-text">User redirected here after successful payment</div>
                            </div>

                            <!-- Cancel URL -->
                            <div class="col-md-6">
                                <label for="cancelUrl" class="form-label fw-semibold"> <i class="bi bi-x-circle me-1"></i>Cancel URL (Failed) </label>
                                <input type="url" id="cancelUrl" formControlName="cancelUrl" class="form-control" />
                            ...
                            <div class="col-md-6 d-flex align-items-end">
                                <div class="form-check form-switch">
                                    ...
                                    <label class="form-check-label" for="isActive"> <i class="bi bi-power me-1"></i>Active </label>
                                </div>
                            </div>
                        </div>

                        <div class="d-flex gap-2 mt-4">
                            <button type="submit" [disabled]="!configForm.valid || saving" class="btn btn-primary">
                                <i class="bi bi-{{ saving ? 'arrow-repeat spin' : 'save' }} me-1"></i>
                                {{ saving ? 'Saving...' : editingConfig ? 'Update' : 'Create' }}
                            </button>
                            <button type="button" (click)="cancelEdit()" class="btn btn-secondary"><i class="bi bi-x-lg me-1"></i>Cancel</button>
                        </div>
                    </form>
                </div>
            </div>
                        ...
                        <table class="table table-hover mb-0">
                            ...
                            <tbody>
                                <tr *ngFor="let config of configs">
                                    ...
                                    <td>
                                        <button (click)="toggleActive(config)" [class]="config.isActive ? 'badge bg-success border-0' : 'badge bg-secondary border-0'" style="cursor: pointer;">
            ...
            <div class="alert alert-info mt-4" role="alert">
                <h5 class="alert-heading"><i class="bi bi-info-circle-fill me-2"></i>Configuration Guide</h5>
                <hr />
                <div class="mb-3">
...
export class PaymentGatewayConfigComponent implements OnInit {
    ...
    saveConfig(): void {
        ...
        const request = this.editingConfig ? this.paymentConfigService.gatewayUpdate(this.editingConfig.id!, dto) : this.paymentConfigService.gatewayCreate(dto);
